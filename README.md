...
**Headline finding.** Across three independent tests, **severe thunderstorm** warnings are the one phenomenon where 2025 stands out beyond ordinary year-to-year variation, a coherent shift toward more and larger warnings (probability of detection up, false alarms up, lead time steady, polygons larger), concentrated geographically. Tornado and flash flood show isolated or no system-level anomalies. The result is a documented, reproducible signal that warrants scrutiny, not a finding of cause.

The full write-up lives in [`notebooks/06_synthesis.ipynb`](notebooks/06_synthesis.ipynb) (the draft report, Sections I–X).
...
- Collected in two phases: 2020–2025 on May 22, 2026 (732 files); the 2016–2019 baseline backfill on June 6, 2026 (488 files). 2025 is a complete calendar year collected well after year-end, with no partial-year bias.
...
│   ├── extraction/   COWExtractor (JSON → CSVs), extract_ugc.py (county codes)
...
├── img/           The 13 figures embedded in the report (regenerated by 04 and 05)
...
│                  (slide_outline.md, build_slides.py, slides.html, reveal/)
├── README.md      This file
...
Standard scientific-Python dependencies are assumed (pandas, numpy, scipy, statsmodels, matplotlib, seaborn, requests). The spatial test in `05_analysis.ipynb` additionally requires `h3` (v4), `geopandas`, and `shapely`, and its maps fetch US Census TIGER state boundaries over the network at run time.
...
- The published deposit reflects data collected in two phases (2020–2025 on May 22, 2026; the 2016–2019 backfill on June 6, 2026); 2025 is a complete calendar year with no partial-year bias at those dates.
