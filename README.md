...
8. **Storm-mode covariates:** `svr_tornado_possible` and `tor_in_svrtorpossible` are SV-warning-specific booleans set by the forecaster at issuance time. They serve as direct triage indicators in the regression: an SV warning with `svr_tornado_possible=True` means the forecaster was simultaneously tracking a potential tornado — a condition where prioritization pressure is highest. `hailtag`/`windtag` partially address the storm-mode confound (a more intense storm is more likely to verify regardless of staffing). `carea`/`parea`/`sharedborder` are warning geometry fields that proxy for precautionary issuing behavior and outbreak density. All seven fields are SV-centric; for TO and FF warnings these columns are structurally null or zero.
