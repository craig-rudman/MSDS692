...
- `docs/` — source corpus: PDFs and matching `.txt` extractions for every cited work (peer-reviewed papers, government reports, executive orders, journalism), plus `literature.md`, the annotated bibliography and the home for full reference details. New downloaded sources follow the `YYYY-MM-DD-outlet-slug.pdf` convention with a paired `.txt`. Keep both formats: the **PDF is the citable archival object** (references point at it; it is the hedge against link rot), and the **`.txt` is the working surface** for reading, searching, and quoting. Favor the `.txt` when working, cite the PDF. Because text extraction can drop or mangle content, **verify any load-bearing material (direct quotes, the exact wording of a Congressional letter or an official denial, dates, figures) against the source PDF before relying on it.** Sourcing standard: be as careful as possible; do not assert a claim the corpus cannot support.
