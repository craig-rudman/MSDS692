...
- Use clustered or robust standard errors. Storms cluster, so observations are not fully independent. Cluster at the storm or outbreak level; naive standard errors understate uncertainty. Demonstrated for severe storm (the worst case) in the appendix of 05_analysis: `src/analysis/cluster.py` builds weather outbreaks (offices linked within 300 km on a noon-to-noon convective day) and refits the three SV models with cluster-robust standard errors, which inflate every SV standard error about three to four times without moving the point estimate; SV FAR still survives (p about 0.005 rather than the naive 1e-17). The 300 km radius and convective-day cutoff are stated choices. Extending this to all nine fits, with threshold sensitivity and a multiple-comparison pass, is the remaining work.
